...
#
    ...
    for i in text:
        ...
        linkMsg=f"""[{{author}} {{short_hash}}](https://github.com/duolabmeng6/GoEasyDesigner/commit/{{hash}}) {{date}}"""
        message = message + linkMsg.format(author=author, short_hash=short_hash, hash=full_hash, date=date)

        result.append({'author': author, 'message': message, 'short_hash': short_hash, 'time': date, 'full_hash': full_hash})
...
{{变更内容}}"""
    print(textTMP.format(用了多少时间=hours, 最新发布信息=最新发布信息, 变更内容=变更内容))
